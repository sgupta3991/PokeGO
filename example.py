...
def get_args():
    ...
    parser.add_argument('-u', '--username', help='Username', default='sethjones91', required=False)
    parser.add_argument('-p', '--password', help='Password', default='Liberty93', required=False)
    parser.add_argument(
        '-l', '--location', type=parse_unicode, help='Location', required=False)
    parser.add_argument('-st', '--step-limit', help='Steps', default='20', required=False)
...
def main():
    ...
    if not (FLOAT_LAT and FLOAT_LONG):
		...
		if args.location is not None:
			...
			retrying_set_location(getLoc())

    if args.auto_refresh:
        global auto_refresh
		#might change this
        auto_refresh = int(args.auto_refresh) * 100
